...
$(document).on('click','.add-to-cart',function(e){
  ...
  //---------------------------------------------------------
  if($(this).hasClass('disable')){    //disable double klik
    return false;
  }
  ...
  var posTop = parent.offset().top;    //return ke koordinat elemen
  ...
  setTimeout(function(){
    ...
    $(".total-amount").text(function(){
     for(var i in bag){
      total += bag[i];    //Menghitung semua jumlah total tiket
     }
...
$(document).ready(function() {
  ...
  $('#end').on('click', function(event) {
    ...
    // hitung diskon
    var totalPrice = parseFloat(last.replace("$ ", ""));
...
$(document).ready(function(){
  $('#coupon').on('click',function(){
    alert( "Diskon Jika Anda Member Tetap" + '\n'+"30% off: 058" )
  });
})
